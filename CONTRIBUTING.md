...
#### Step 2: Run the Parser (Optional)

**Good news!** 🎉 The parser runs automatically via GitHub Actions when you push or merge to master. However, you can test locally:
...
**Note:** If you don't run the parser locally, GitHub Actions will automatically generate the JSON files when your PR is merged.
...
5. **(Optional)** Run `npm run parse-readme` to test locally
6. **(Optional)** Test your changes with `npm run dev`
7. Commit **only README.md** (JSON files will be auto-generated)
8. Push to your fork
